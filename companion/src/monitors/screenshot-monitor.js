...
class ScreenshotMonitor {
  ...
  start(callback) {
    ...
    if (fs.existsSync(this.screenshotPaths.workspaceImages)) {
      ...
      console.log(
        `[SCREENSHOT] Monitoring workspace screenshots: ${this.screenshotPaths.workspaceImages}`
      );
    }
      ...
      if (this.processImages) {
        try {
          const processed = await this.imageProcessor.processImage(filePath, {
            ...
            computeHash: true,
            skipValidation: false, // Use validation to catch invalid files early
          });

          // Only add metadata if processing was successful
          if (processed.success && !processed.skipped) {
            ...
            if (processed.thumbnail && processed.thumbnail.success) {
            ...
            // File was skipped (invalid format, corrupted, etc.) - this is expected
            // Don't log as error, just mark it
            screenshotData.processingSkipped = true;
            screenshotData.skipReason = processed.error;
          }
        } catch (error) {
